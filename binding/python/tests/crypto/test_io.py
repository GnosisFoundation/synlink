...
def test_io_read_ssh_file(file_buffer: tempfile._TemporaryFileWrapper):
    try:
        _ = file_buffer
        _keypair = load_ssh_private_key(_.name, password="hello-world")
    # finally close the file
    finally:
        _.close()


def test_io_invalid_read_ssh_file(
    invalid_file_buffer: tempfile._TemporaryFileWrapper,
):
...
def test_io_invalid_key_read_ssh_file(
    file_buffer: tempfile._TemporaryFileWrapper,
):
    _ = file_buffer

    with pytest.raises(Exception):
        _keypair = load_ssh_private_key(_.name, password="hello")
