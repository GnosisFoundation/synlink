...
ALL_VALID: Final[Callable[[List[Multiaddr]], bool]] = lambda expected: all(
    map(lambda x: is_valid_address(x), expected)
)
ASSERT_ALL_VALID_MESSAGE: Final[
    str
] = "The expected address should be valid synlink address."
...
def test_valid_synlink_multiaddr_p2p():
    ...
    ip4_udp = Multiaddr(
        "/ip4/0.0.0.0/udp/0/p2p/QmYyQSo1c1Ym7orWxLYvCrM2EmxFTANf8wXmmE7DWjhx5N"
    )
    ip6_udp = Multiaddr(
        "/ip6/::1/udp/0/p2p/QmYyQSo1c1Ym7orWxLYvCrM2EmxFTANf8wXmmE7DWjhx5N"
    )
    ...
    ip6_tcp = Multiaddr(
        "/ip6/::1/tcp/0/p2p/QmYyQSo1c1Ym7orWxLYvCrM2EmxFTANf8wXmmE7DWjhx5N"
    )
